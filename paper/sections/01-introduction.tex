...
neural devices. The FDA's 2023 cybersecurity guidance for medical
devices~\cite{fda2023cybersecurity} and Section 524B of the FD\&C
Act~\cite{fda524b} address connected medical devices generally, but contain no
...
94.4\% require scoring dimensions that CVSS cannot express.
...
\begin{enumerate}
  ...
  \item \textbf{TARA Threat Taxonomy} (Section~\ref{sec:tara}): A registry of
        ...
        evidence status, severity, and dual-use therapeutic potential. TARA extends
        the MITRE ATT\&CK\textsuperscript{\textregistered} methodology to the BCI
        domain.

  \item \textbf{NISS Scoring System} (Section~\ref{sec:niss}): The Neural Impact
        Scoring System---a CVSS v4.0 extension following FIRST.org's official
        extension mechanism (User Guide \S3.11)~\cite{first2023cvss4userguide}
        that adds five neural-specific metrics. NISS has been submitted to the
        FIRST.org CVSS Special Interest Group for formal registration.
