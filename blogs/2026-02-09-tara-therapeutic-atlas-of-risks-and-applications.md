...
<script>
(function(){
  const el = document.getElementById('derivation-timeline');
  if (!el) return;
  ...
  fetch('/data/derivation-timeline.json')
    ...
    .then(data => {
      ...
      groups.forEach((g, gi) => {
        ...
        const nodeColor = cats[topItem.category]?.color || '#06b6d4';
        const node = document.createElement('div');
        const isAha = g.items.some(i => i.type === 'aha');
        const size = g.items.length > 1 ? '16px' : '12px';
        ...
        const techItem = g.items.find(i => i.metrics?.techniques);
      ...
      const last_m = ms[ms.length - 1];
      const stats = [
        last_m.metrics?.log_entries + ' entries',
        last_m.metrics?.days_of_derivation ? last_m.metrics.days_of_derivation + ' days' : null,
        last_m.metrics?.framework_versions ? last_m.metrics.framework_versions + ' versions' : null,
