...
**TL;DR:** People with spinal cord injuries are about to get artificial replacements for their lost reflex pathways — brain-spine interfaces that bridge the gap with digital signals. This is already happening in human trials. But the replacement has to be fast, deterministic, and hardwired to meet the same latency constraints the biological spinal cord did. That means it inherits the fragility tax: cheap to run, cheap to break. Except now the attack surface is digital, not biological. B12 deficiency is slow, hard to weaponize, and affects one person. A corrupted reflex table in a neural prosthetic is instant, targetable, and potentially scalable. The people who need this technology most are the people most vulnerable to its failure mode. We need to talk about that before, not after, it ships.
